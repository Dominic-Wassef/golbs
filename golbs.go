...
// golbsFile temp file that stores the modification time of the build script.
var golbsFile string = fmt.Sprintf("%s/golbs_file", os.TempDir())
...
func storeModTime(buildFile string) (bool, error) {
	...
	_, err = os.Stat(golbsFile)
	if os.IsNotExist(err) {
		if err := os.WriteFile(golbsFile, []byte(fileInfo.ModTime().String()), 0666); err != nil {
			return false, err
		}
		return false, nil
	}

	data, err := os.ReadFile(golbsFile)
	if err != nil {
		return false, err
	}
